...
public class MajorController {
    ...
    public String list(){
        return "view/param/zk/list";
    }

    @RequiresPermissions("sys:role:list")
    @PostMapping("list")
    @ResponseBody
    public PageData<ZkMajor> list(@RequestParam(value = "page",defaultValue = "1")Integer page,
                                @RequestParam(value = "limit",defaultValue = "10")Integer limit,
                                ServletRequest request){
        Map map = WebUtils.getParametersStartingWith(request, "s_");
        PageData<ZkMajor> rolePageData = new PageData<>();
        QueryWrapper<ZkMajor> majorWrapper = new QueryWrapper<>();
//        majorWrapper.eq("del_flag",false);
        if(!map.isEmpty()){
            String keys = (String) map.get("key");
            if(StringUtils.isNotBlank(keys)) {
                majorWrapper.like("pname", keys);
            }
        }
        IPage<ZkMajor> majorPage = zkMajorService.page(new Page<>(page,limit),majorWrapper);
        rolePageData.setCount(majorPage.getTotal());
        rolePageData.setData(majorPage.getRecords());
        return rolePageData;
    }
